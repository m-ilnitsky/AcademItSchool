...
public class ConsoleView implements View {
    ...
    private void showStatusBar() {
        long time = core.getGameTime();
        int numActions = core.getNumActions();
                    ...
                    if (isError) {
                        ...
                        if (isFlag || isQuery) {
                            if (cellState != CellState.CLOSE && cellState != CellState.FLAG && cellState != CellState.QUERY) {
                                ...
                                if (isFlag) {
                                    core.setFlag(xPosition, yPosition);
                                    gameStatus = core.getGameStatus();
                                } else {
                                    core.setQuery(xPosition, yPosition);
                                    gameStatus = core.getGameStatus();
                                }
                            ...
                            if (cellState == CellState.CLOSE || cellState == CellState.FLAG || cellState == CellState.QUERY) {
                                ...
                                core.setOpenAllAround(xPosition, yPosition);
                                gameStatus = core.getGameStatus();
                                lastCommands.add(line);
                            }
                        } else {
                            if (cellState == CellState.CLOSE || cellState == CellState.QUERY) {
                                core.setOpen(xPosition, yPosition);
                                gameStatus = core.getGameStatus();
                                lastCommands.add(line);
                            } else {
    ...
    private void showSaveResult() {
        ...
        boolean isTop = core.isTopScores(gameInfo);
    ...
    private void showTopScores(GameSize gameSize) {
        boolean isTop = core.isTopScores(gameSize);
        GameInfo[] gameArray;
        String gameName;

        if (isTop) {
            gameArray = core.getTopScores(gameSize);
            gameName = core.getTopScoresName(gameSize);
